...
task={'D1', 'D2', 'G1', 'G2'}; % two runs simple RT and two runs gng
...
%                 exponent_prestim{t}(index2delete ) = []; 
    ...
    %         continuous_variable(:, 2) = zscore([alpha_power{1}; alpha_power{2}; alpha_power{3}; alpha_power{4}]);
            ...
            continuous_variable(:, 4) = [FCz_erp_bslnamp{task_runs(tsk, 1)}.*sin(FCz_erp_bsln{task_runs(tsk, 1)}); FCz_erp_bslnamp{task_runs(tsk, 2)}.*sin(FCz_erp_bsln{task_runs(tsk, 2)})];
...
save eeg_model_rsquared_per_task eeg_model_rsquared
save betas_time_alphaPOz_EEGbslnampphase_per_task_young betas_young
save betas_time_alphaPOz_EEGbslnampphase_per_task_older betas_older
save erp_resd_alpha_bslnampphase_per_task_std erp_resd_alpha_bslnphase_std
...
% run, time-on-task, alpha, pupil, eeg bsln
for task = 2%1:2
    [h,p,ci,stats] = ttest(betas_young{task})

    [h,p,ci,stats] = ttest(betas_older{task})


    [h,p,ci,stats] = ttest2(betas_young{task}, betas_older{task})


    [h,p,ci,stats] = ttest([betas_young{task}; betas_older{task}])
end

%% graphs of coefficients
cd('G:\ProjectAgingNeuromodulation\AuditoryResearch\EEGLAB_analysis\ERP_variability')
load betas_time_alphaPOz_EEGbslnampphase_per_task_young;
load betas_time_alphaPOz_EEGbslnampphase_per_task_older
...
for task = 2%1:2
    for coef = 1:5
        ...
        plot_all_data_2groups(betas_young{task}(: , coef+1), betas_older{task}(: , coef+1), y_label_text, title_text(coef))
    end
end
...
for task  = 1:2
    ...
    mean(eeg_model_rsquared{task, 1}(:, 1))
    std(eeg_model_rsquared{task, 1}(:, 1))
    
    mean(eeg_model_rsquared{task, 2}(:, 1))
    std(eeg_model_rsquared{task, 2}(:, 1))
end

%% erp_residuals_total_std{grp}
for task =1:2
    plot_all_data_2groups(erp_resd_alpha_bslnphase_std{task, 1}, erp_resd_alpha_bslnphase_std{task, 2}, 'ERP residuals std', '')
end
...
%   without clearing existing content from the axes.
   ...
   rectangle('Position',[2-0.3,yMean-y_se, 0.6, 2*y_se ],'FaceColor',[.7 .7 .7],'EdgeColor', [.7 .7 .7],'LineWidth',0.1)
    ...
    for y=1:length(data_grp2)
        plot(2+rand*0.2-0.1, data_grp2(y) ,'-o',  'color', [1 .5 .5], ...
