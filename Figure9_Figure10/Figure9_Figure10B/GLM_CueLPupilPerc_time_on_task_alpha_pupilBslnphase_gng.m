...
%        epochs_erp
    ...
    % create continuous variable with time-on-task; alpha baseline; pupil phase cos; pupil phase sin - total 45 continuous variables
...
%         continuous_variable(:, 2) = zscore([alpha_power{1}; alpha_power{2}; alpha_power{3}; alpha_power{4}]);
        ...
        if grp == 1
            ...
            % trial-by-trial variability in pre-stim alpha and pupil
            alpha_std_young = [alpha_std_young; std(continuous_variable(:, 2))];
        else
            betas_older = [betas_older; g.Coefficients.Estimate'];
            % trial-by-trial variability in pre-stim alpha and pupil
            alpha_std_older = [alpha_std_older; std(continuous_variable(:, 2))];
        end
        
    end
end

save betas_alpha_pupilampphase_gng_young betas_young
save betas_alpha_pupilampphase_gng_older betas_older
% save alpha_std_young alpha_std_young
% save alpha_std_older alpha_std_older 
save pupil_ampphaseresid_std_gng pupil_resid_std
save pupil_model_rsquared_gng pupil_model_rsquared
...
%% plot pupil residuals std

% plot_all_data_2groups(data_grp1, data_grp2, y_label_text)
plot_all_data_2groups(pupil_resid_std{1}, pupil_resid_std{2},'PD residuals SD', '')

%% plot model Rsquared
load pupil_model_rsquared_gng
% plot_all_data_2groups(data_grp1, data_grp2, y_label_text)
plot_all_data_2groups(pupil_model_rsquared{1}(:, 1), pupil_model_rsquared{2}(:,1),'Model R squared', '')

plot_all_data_2groups(pupil_model_rsquared{1}(:, 2), pupil_model_rsquared{2}(:, 2),'Model R squared adjusted', '')
...
mean(pupil_model_rsquared{1}(:, 1)); std(pupil_model_rsquared{1}(:, 1))

mean(pupil_model_rsquared{2}(:, 1))
std(pupil_model_rsquared{2}(:, 1))
