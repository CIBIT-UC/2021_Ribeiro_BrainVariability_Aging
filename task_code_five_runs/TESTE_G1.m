% Cued Auditory Task - go/no-go condition
...
imperative_keys={}; 
imperative_times=[];
nogo_keys={};% was changed to cell after subject 13
nogo_times=[];
blank_keys={};% was changed to cell after subject 13
blank_times=[];
...
% Exponential random number using function exprnd - number of trials 50
...
NoGoBeep=MakeBeep(1300, beepLengthSecs, freq)*0.2;

% Make Beep for slow responses
SlowResponsesBeep = MakeBeep(1000, beepLengthSecs, freq)*0.3;
% Make beep for incorrect response to no-go stimulus
ImpulsiveResponsesBeep = MakeBeep(1000, beepLengthSecs, freq)*0.3;

% order of presentation of go and no-go stimuli
% 1 - go stimuli; 2 - no-go stimuli; 3 - blank no imperative stimulus
stimuli_order=[];
target_flanking=[1 1];
nogo_order=[1 1 2 3];

for g=1:10
    stimuli_order=[stimuli_order target_flanking Shuffle(nogo_order)];
end


% to get out of Psychtoolbox if an error occurs
% try
...
WaitSecs(5);
    % send triggers eye tracker
            %TRIGGERS EEG
            lptwrite(address, 9);
            WaitSecs(0.004);
            lptwrite(address, 0);
            iViewXComm('send', ivx, 'ET_AUX "StarREC_gng"');
WaitSecs(5);
...
% exponential distribution random generated numbers - minimum 1 sec 
...
elseif stimuli_order(t)==2; % play no-go stimuli
PsychPortAudio('FillBuffer', pahandle, [NoGoBeep;NoGoBeep]);
...
PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);
target_time(t)=GetSecs;
                            %TRIGGERS EEG ET - no-go
                            lptwrite(address, 4);
                            WaitSecs(0.004);
                            lptwrite(address, 0);
                            iViewXComm('send', ivx, strcat('ET_AUX "nogo"'));%trigger eye tracker
    ...
    while GetSecs-target_time(t)<1.2; %wait 5 secs before next stimulus
                        ...
                        if keyIsDown==1
                            ...
                            WaitSecs(0.004);
                            lptwrite(address, 0);
                            iViewXComm('send', ivx, strcat('ET_AUX "nogoresponse"'));
                            ...
                            nogo_keys{t, count}=key;
                            nogo_times(t, count)=secs;
                        end
    end

        % play error sound for impulsive responses
        if size(nogo_times,1)<t;
        nogo_times(t, 1)=-1; % correct withhold of response

        elseif size(nogo_times,1)==t;
        % Put sound in buffer   
        PsychPortAudio('FillBuffer', pahandle, [ImpulsiveResponsesBeep;ImpulsiveResponsesBeep]);
        % Start audio playback
        PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);
                            %TRIGGERS EEG impulsive response
                            lptwrite(address, 7);
                            WaitSecs(0.004);
                            lptwrite(address, 0);
...
elseif stimuli_order(t)==3; % do not play target stimulus
target_time(t)=GetSecs;    
   %count number of responses
count=0;
key=[]; 
    ...
    PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);
        ...
        elseif size(blank_times,1)==t; % subject incorrectly pressed response button - no feedback!!!
        % Put sound in buffer   
%         PsychPortAudio('FillBuffer', pahandle, [ImpulsiveResponsesBeep;ImpulsiveResponsesBeep]);
%         Start audio playback
%         PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);
...
cd(strcat(pwd, '\G1'));
 ...
 save nogo_times nogo_times
 save nogo_keys nogo_keys
