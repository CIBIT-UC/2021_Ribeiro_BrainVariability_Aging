% Cued Auditory Task - warning only
...
WaitSecs(5);
             ...
             WaitSecs(0.004);
             lptwrite(address, 0);
             iViewXComm('send', ivx, 'ET_AUX "StartREC_detection"');% trigger ET
...
elseif imperative_times(t, 1)>0.7;
    % Put sound in buffer   
    PsychPortAudio('FillBuffer', pahandle, [SlowResponsesBeep;SlowResponsesBeep]);
    % Start audio playback
    PsychPortAudio('Start', pahandle, repetitions, startCue, waitForDeviceStart);
...
%                             %TRIGGERS PARA A RESPOSTA
        ...
        elseif size(blank_times,1)==t; % subject incorrectly pressed response button
        % Put sound in buffer   
